...
const tuiguang = '农场红包没地方用？看看这里有没有需要的：<a href="https://u.jd.com/9zVyrZF">点击查看</a>';
// =======================================go-cqhttp通知设置区域===========================================
...
if (process.env.WP_APP_TUIGUANG) {
    tuiguang = process.env.WP_APP_TUIGUANG;
}
let WP_UIDS_ONE = "";
...
async function sendNotify(text, desp, params = {}, author = "\n================================\n" + tuiguang, strsummary = "") {
    ...
    try {
        ...
        if (text.indexOf("已可领取") != -1) {
            if (text.indexOf("农场") != -1) {
                strTitle = "东东农场领取";
                // if(text.indexOf('请去京东APP或微信小程序查看')!==-1) {
                //     var strPtPin = await GetPtPin(text);
                //     var strdecPtPin = decodeURIComponent(strPtPin);
                //     if (strPtPin) {
                //         console.log(`${strTitle} ${strdecPtPin}一对一通知`);
                //         await sendNotifybyWxPucher(strTitle, text, strdecPtPin);
                //     }
                // }
            } else {
                strTitle = "东东萌宠领取";
    ...
    await Promise.all([
        ...
        bncrNotify(text, desp, author, envs), //bncr
...
function bncrNotify(text, desp, author, envs) {
    desp = desp.replace(author, '').replace(/【京东账号(\d+)】/g, '【账号】').replace(/【账号(\d+)】/g, '【账号】')
    if (text.indexOf('京东白嫖提醒') !== -1) {
        desp = processAccountInfo(desp)
    } else {
        for (let i = 0; i < envs.length; i++) {
            let temppin = decodeURIComponent(envs[i].value.match(/pt_pin=([^; ]+)(?=;?)/) && envs[i].value.match(/pt_pin=([^; ]+)(?=;?)/)[1]);
            if (desp.indexOf(temppin) !== -1) {
                // 将tempptpin中间字符串替换为*
                desp = desp.replace(temppin, maskString(temppin, 6, 4))
            }
        }
    }
    return new Promise(resolve => {
...
function processAccountInfo(accountInfo) {
    if (accountInfo.includes('京东账号')) {
        return accountInfo.replace(/【京东账号(\d+)\s*(\S+)】/g, (match, p1, p2) => {
            return `【账号 ${maskString(p2, 2, 4)}】`;
        });
    }
    return accountInfo.replace(/【账号(\d+)\s*(\S+)】/g, (match, p1, p2) => {
        return `【账号 ${maskString(p2, 2, 4)}】`;
    });
}

function maskString(str, frontLen, backLen) {
    if (str.length <= frontLen + backLen - 1) {
        return mask(str, 1, 1)
    } else {
        return mask(str, frontLen, backLen)
    }
}

function mask(str, frontLen, backLen) {
    // 检查字符串长度是否足够，如果不够，直接返回原字符串
    if (str.length <= frontLen + backLen) {
        return str;
    }

    // 提取前frontLen个字符和最后backLen个字符
    const start = str.substring(0, frontLen);
    const end = str.substring(str.length - backLen);

    // 中间部分替换为星号
    const middle = '*'.repeat(str.length - (frontLen + backLen));
